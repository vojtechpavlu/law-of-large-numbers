...
const IterationController = ({state, dispatch}) => {
  ...
  const handleDecrease = () => {
    if (state.iterations > 1) {
      dispatch({type: ACTIONS.SET_ITERATIONS, payload: {iterations: Math.floor(state.iterations / 10)}})
    }
  }

  const handleIncrease = () => {
    if (state.iterations <= 100_000) {
      dispatch({type: ACTIONS.SET_ITERATIONS, payload: {iterations: state.iterations * 10}})
    }
  }
  ...
  return (
    <div style={STYLE}>
      ...
      <div>
        ...
        >
          <span className="lead"><strong>-</strong></span>
        </Button>
